...
static void localsockfn(struct eventloop *el, int op, void *arg) {
  ...
  case OP_LOCALAPI_RECV_MSG:
    ...
    if ( FD_WRITE_AVAILABLE(fde) ) {
      fprintf(stderr, "Need to do write\n");
      if ( localsock_flush(api, el) < 0 ) {
...
struct localapi *localapi_alloc(struct appstate *as, int sk) {
  ...
  FDSUB_SUBSCRIBE(&ret->la_socket_sub, FD_SUB_READ);
  eventloop_subscribe_fd(&as->as_eventloop, ret->la_socket, &ret->la_socket_sub);
...
static void localsock_hup(struct localapi *api, struct eventloop *el) {
  ...
  if ( api->la_socket ) {
    eventloop_unsubscribe_fd(el, api->la_socket, &api->la_socket_sub);
    close(api->la_socket);
  }
...
static int localsock_respond(struct localapi *api, struct eventloop *el,
  ...
  *sz = buf_sz;
  ...
  FDSUB_SUBSCRIBE(&api->la_socket_sub, FD_SUB_WRITE);
  eventloop_subscribe_fd(el, api->la_socket, &api->la_socket_sub);
...
static void localsock_crud_flock(struct localapi *api, struct eventloop *el,
  ...
  case KLM_REQ_GET:
    assert(pthread_rwlock_rdlock(&api->la_app_state->as_flocks_mutex) == 0);
...
static void localsock_handle_message(struct localapi *api, struct eventloop *el,
  ...
  case KLM_REQ_ENTITY_APP:
    localsock_return_error_response(api, el, msg, KLE_NOT_IMPLEMENTED);
    break;
...
static int localsock_flush(struct localapi *api, struct eventloop *el) {
  ...
  if ( (outgoing_ptr + 4) < api->la_outgoing_sz ) {
    ...
    FDSUB_UNSUBSCRIBE(&api->la_socket_sub, FD_SUB_WRITE);
