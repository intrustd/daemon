...
// Start log-in process, must hold the mutex
...
static void connectionfreefn(const struct shared* s, int level) {
  ...
  if ( level == SHFREE_NO_MORE_REFS ) {
    assert( pthread_mutex_lock(&conn->conn_mutex) == 0 );
...
int connection_start_authentication(struct connection *conn) {
  struct applianceinfo *app = connection_get_appliance(conn);
  int ret = 0;
...
static void connection_reset_retry_timer(struct connection *conn) {
  timersub_set_from_now(&conn->conn_ai_retry_connect, CONNECTION_APPLIANCE_TIMEOUT << conn->conn_ai_retries);
  if ( conn->conn_el ) {
...
static void connection_do_request_sent(struct connection *conn) {
  if ( conn->conn_control(conn, CONNECTION_OP_APP_REQ_SENT, NULL) < 0 ) {
    ...
    connection_reset_retry_timer(conn);
  }
}
...
void connection_offer_received(struct connection *conn, connofferlnfn lines, void *ud) {
  int next_line, err, finish = 0;
  struct sdpln ln;

  fprintf(stderr, "connection offer received\n");
  ...
  if ( finish ) connection_complete(conn);
}
...
void connection_complete(struct connection *conn) {
  ...
  completing = conn->conn_ai_state != CONN_AI_STATE_COMPLETE;
