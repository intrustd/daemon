...
in { name = "uwsgi-${name}";
     startExec = ''
       ${getBin uwsgiWithPlugins}/bin/uwsgi -s ${socket} --chmod-socket=664 --manage-script-name --mount /=${module} --plugin python3 --uid root --processes ${builtins.toString processes} ${optionalString (http != null) "--http ${http}"} ${extraOptions}
     '';
